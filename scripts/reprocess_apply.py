...
def apply_reprocess(bank=None, start_date=None, end_date=None, category=None, only_outros=False, limit=1000, out_path=None, dry_run=False):
    with app.app_context():
        ...
        txs = query.limit(max(1, min(int(limit), 10000))).all()
        if not txs:
            ...
            if new != (t.category or 'NULL'):
                ...
                if len(examples) < 50:
                    examples.append({
                        ...
                        'old': t.category if dry_run else old,  # show original
                        'new': new,
                    })
            else:
                unchanged += 1
        ...
        lines = []
        lines.append('# Reprocessamento de Categorias - Aplicado' + (' (Dry Run)' if dry_run else ''))
        lines.append('')
        ...
        if not dry_run:
            lines.append(f'- IDs atualizados: {", ".join(map(str, updated_ids[:100]))}' + (' ...' if len(updated_ids) > 100 else ''))
        if new_categories:
        ...
        if out_path is None:
            ...
            out_path = os.path.join(docs_dir, 'REPROCESSAMENTO_CATEGORIAS_APLICADO.md')

        with open(out_path, 'w', encoding='utf-8') as f:
            f.write(report_md)

        print(('Dry run ' if dry_run else '') + f'Report saved to: {out_path}')
        if not dry_run:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Apply reprocess categories and generate report markdown')
